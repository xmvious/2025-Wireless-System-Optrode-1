...
#define FLAG_PACKED12           0x02   /* 12-bit packed payload 표시 */
 ...
 * BLE 안정성을 위해 10000 SPS 기준으로 튜닝
 */
#define SAMPLES_PER_FRAME       16     /* 기본 설명용 값 (실제 SPF는 adaptive_spf() 사용) */
#define FRAME_PAYLOAD_INT16     (NUM_CHANNELS * SAMPLES_PER_FRAME)

/* 목표 총 샘플레이트(4채널 합) - 10000 SPS 목표 (채널당 2500) */
#define TARGET_TOTAL_SPS        10000
...
/* 혼잡 완화 백오프(ms) + 연속 성공 카운터 */
static uint32_t backoff_ms = 0;
static uint8_t  succ_streak = 0;
...
static struct perf_stats perf_stats = {0};

/* 전송 큐 구조체 (향후 확장용, 현재 통계용 변수만 사용) */
struct tx_queue_item {
    uint8_t data[1024];
    uint16_t len;
    uint16_t seq;
    bool valid;
    k_timeout_t timestamp;
};

/* 전송 큐 변수들 */
static struct tx_queue_item tx_queue[8];
static int tx_queue_head = 0;
static int tx_queue_tail = 0;
static int tx_queue_count = 0;
static K_MUTEX_DEFINE(tx_queue_mutex);
...
static inline int adaptive_spf(void)
{
    ...
    if (att_mtu >= 247) return 32;  /* DLE 251급 */
    if (att_mtu >= 100) return 24;  /* 중간 MTU */
    return 16;                      /* 작은 MTU/초기 */
}
...
static int nus_send_chunked(const uint8_t *p, uint16_t len)
{
    while (len) {
        ...
        /* 마지막 청크일 때만 1ms 정도 페이싱 */
        if (len == 0) {
            (void)k_sem_take(&nus_write_sem, K_MSEC(1));
        }
    }
    return 0;
}

/* === 프레임 '생성 시' seq 증가 (손실 집계 정확) === */
...
/* ============================================================
 *          ★ 12-bit 패킹 유틸리티 (신규 추가)
 * ============================================================*/
/* 두 개의 12-bit 샘플 x,y를 3바이트로 패킹 (little-endian) */
...
/* burst(인터리브된 int16) → packed 바이트스트림
   total_i16 = NUM_CHANNELS * SPF  (짝수 보장 권장) */
static uint16_t pack_12bit_stream(const int16_t *in_i16, uint16_t total_i16, uint8_t *out_bytes, uint16_t out_cap)
{
    if (total_i16 & 1) total_i16--; /* 짝수 정렬(2샘플 단위) */
    uint32_t need = (uint32_t)total_i16 * 3 / 2;
    if (need > out_cap) return 0; /* capacity 부족 */

    const uint16_t *p = (const uint16_t*)in_i16;
    uint8_t *q = out_bytes;

    for (uint16_t i = 0; i < total_i16; i += 2) {
        uint16_t a = (uint16_t)p[i]   & 0x0FFF;
        uint16_t b = (uint16_t)p[i+1] & 0x0FFF;
        pack2x12(a, b, q);
        q += 3;
    }
    return (uint16_t)((uintptr_t)q - (uintptr_t)out_bytes); /* 실제 바이트 수 */
}

/* ============================================================
 *          버스트 샘플링(폴링) → 프레임 전송 (+페이싱)
 * ============================================================*/
static inline k_timeout_t next_interval_timeout(int spf)
{
    /* 프레임 주기(ms) = 1000 * (NUM_CHANNELS*SPF) / TARGET_TOTAL_SPS + backoff */
    /* 10000 SPS 기준: 예) SPF 16 × 4채널 = 64 샘플/프레임 → 6.4ms 간격 */
    float ms = (1000.0f * (NUM_CHANNELS * spf)) / (float)TARGET_TOTAL_SPS;
    if (ms < 5.0f) ms = 5.0f; /* BLE 연결 간격 고려하여 하한 5ms */
    ms += (float)backoff_ms;
    return K_MSEC((int)ms);
}
...
static void schedule_next_adc(void)
{
    ...
    k_work_reschedule(&adc_work, next_interval_timeout(SPF));
}
...
static void read_adc_and_print_all(struct k_work *work)
{
    ...
    int16_t burst_stack[NUM_CHANNELS * 128]; /* 상한 128 SPF 기준 (여유) */
    ...
    for (int n = 0; n < SPF; n++) {
        ...
        for (int ch = 0; ch < NUM_CHANNELS; ch++) {
            /* 12-bit 범위(0..4095) 가정, int16로 보관 */
            burst[n * NUM_CHANNELS + ch] = sample_buf[ch] & 0x0FFF;
        }
    }

    /* ★ 12-bit 패킹 */
    uint8_t payload[(NUM_CHANNELS * SPF * 3) / 2 + 3]; /* 여유 몇 바이트 */
    uint16_t plen = pack_12bit_stream((const int16_t*)burst, (uint16_t)TOTAL_I16, payload, sizeof(payload));
    if (plen == 0) {
        LOG_WRN("pack_12bit_stream overflow or zero");
        adc_in_flight = false;
        schedule_next_adc();
        return;
    }
    ...
    /* ★ FLAG_PACKED12로 전송 */
    int e = send_frame_over_nus(payload, plen, DEFAULT_CH_MASK, FLAG_PACKED12);
    if (e) {
        LOG_WRN("send_frame_over_nus err=%d", e);
        succ_streak = 0;
        if (backoff_ms < 8) backoff_ms += 1;     /* 혼잡 시 점진 증가 (최대 8ms) */
    } else {
        if (++succ_streak >= 1) backoff_ms = 0;  /* 성공 1회 → 즉시 해제 */
    }
...
static void connected(struct bt_conn *conn, uint8_t err)
{
    ...
    struct bt_le_conn_param *param = BT_LE_CONN_PARAM(4, 8, 0, 400); /* 5~10ms */
    int err_cp = bt_conn_le_param_update(conn, param);
    if (err_cp) {
        LOG_WRN("Conn param update failed: %d", err_cp);
    }

    LOG_INF("Connected: %s (MTU %u, chunk %u)", addr, bt_gatt_get_mtu(conn), nus_max_chunk());
}
 ...
 *                       기타 유틸
 * ============================================================*/
/* error 함수는 현재 사용하지 않음 - UART 실패 시에도 BLE로 계속 진행 */
...
static void process_tx_queue(void) {
    /* 현재는 단순한 구현 - 필요시 확장 */
    if (tx_queue_count > 0) {
        /* 큐에 대기 중인 프레임이 있으면 처리 */
        LOG_DBG("Processing TX queue: %d frames", tx_queue_count);
    }
}
 ...
 *              성능 통계 로깅 함수
 * ============================================================*/
static void log_performance_stats(void) {
    static uint32_t last_log = 0;
    uint32_t now = k_uptime_get_32();
    
    if (now - last_log > 5000) {  /* 5초마다 */
        LOG_INF("Perf: sent=%d, dropped=%d, queued=%d, congestion=%d",
                perf_stats.frames_sent, perf_stats.frames_dropped,
                perf_stats.frames_queued, perf_stats.congestion_events);
        last_log = now;
    }
}
...
int main(void) {
    ...
    LOG_INF("Starting OPTIMIZED NUS peripheral...");
    ...
    for (int retry = 0; retry < 5; retry++) {
        err = uart_init_wrap();
        if (err == 0) {
            LOG_INF("UART initialization successful");
            break;
        }
        
        LOG_ERR("UART initialization failed (attempt %d/5): %d", retry + 1, err);
        if (retry < 4) {
            LOG_INF("Retrying UART initialization in 1 second...");
            k_sleep(K_MSEC(1000));
        }
    }
    
    if (err != 0) {
        LOG_ERR("UART initialization failed after 5 attempts, continuing without UART");
        /* UART 없이도 계속 진행 (BLE만 사용) */
        /* LED로 UART 실패 상태 표시 */
    ...
    /* 시작 메시지 (UART가 사용 가능한 경우에만) */
    if (err == 0) {
        struct uart_data_t *tx = k_malloc(sizeof(*tx));
        if (tx) {
            snprintf((char*)tx->data, sizeof(tx->data), "Optimized NUS start (10000 SPS, 12-bit packed)\n");
            tx->len = (uint16_t)strlen((char*)tx->data);
            int tx_err = uart_tx(uart, tx->data, tx->len, 100);
            if (tx_err) {
                LOG_WRN("Startup message TX failed: %d", tx_err);
                k_fifo_put(&fifo_uart_tx_data, tx);
            }
        }
    }
    ...
    printk("OPTIMIZED 4-channel ADC burst to BLE (10000 SPS target, 12-bit packed)\n");
